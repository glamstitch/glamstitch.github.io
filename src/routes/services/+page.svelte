<script lang="ts">
	// Page content only - dark mode handled by layout
	import { base } from '$app/paths';
	import logoGlint from '$lib/assets/image/logo glint.png';
</script>

<svelte:head>
	<title>Layanan - Glam Stitch | Sablon, Bordir & Konveksi Berkualitas</title>
	<meta
		name="description"
		content="Layanan konveksi lengkap dari Glam Stitch: sablon, bordir, konveksi kaos, jersey, jaket, dan kemeja. Kualitas terjamin dengan harga kompetitif."
	/>
	<meta
		name="keywords"
		content="layanan konveksi, sablon, bordir, konveksi kaos, jersey, jaket, kemeja, kualitas terjamin"
	/>
</svelte:head>

<!--header-->
<section class="relative isolate overflow-hidden bg-gray-900 pt-4 dark:bg-gray-950">
	<!-- Background Image -->
	<img
		src="https://img.freepik.com/premium-photo/pensive-young-man-leather-apron-drinking-coffee-while-taking-break-leather-workshop_274679-7615.jpg?uid=R108872572&ga=GA1.1.1868222757.1738365469&semt=ais_hybrid&w=740"
		alt="Layanan Konveksi"
		class="absolute inset-0 -z-10 h-full w-full object-cover opacity-90"
		loading="lazy"
	/>
	<div class="absolute inset-0 -z-10 bg-black/30"></div>

	<!-- Container -->
	<div class="mx-auto max-w-7xl px-6 lg:px-8">
		<div class="mx-auto max-w-2xl py-32 text-center sm:py-48 lg:py-32">
			<!-- Badge Link -->
			<div class="mb-6 hidden justify-center sm:flex">
				<div
					class="relative rounded-full bg-white/10 px-4 py-1.5 text-sm text-gray-200 ring-1 ring-white/20 transition hover:ring-white/40"
				>
					Solusi konveksi terbaik untuk kebutuhan Anda.
					<a href="/product" class="ml-2 font-semibold text-white underline underline-offset-4">
						Lihat Produk <span aria-hidden="true">&rarr;</span>
					</a>
				</div>
			</div>

			<!-- Heading & Content -->
			<div class="text-center">
				<h1 class="text-4xl leading-tight font-bold tracking-tight text-white sm:text-5xl">
					Layanan Konveksi Profesional dari Glam Stitch
				</h1>
				<p class="mx-auto mt-4 max-w-xl text-base font-medium text-gray-300 sm:text-lg">
					Kami menyediakan berbagai layanan produksi pakaian — mulai dari kaos polos, jaket custom,
					hingga kemeja formal — dengan standar kualitas tinggi dan pengerjaan cepat.
				</p>
			</div>
		</div>
	</div>
</section>

<!-- CTA Section -->
<section class="bg-white py-24 dark:bg-gray-900">
	<div class="mx-auto max-w-7xl px-6 lg:px-16">
		<div class="mb-16 text-center">
			<h2 class="text-4xl font-bold tracking-tight text-gray-800 sm:text-5xl dark:text-white">
				Layanan Unggulan Glam Stitch
			</h2>
			<p class="mt-4 text-lg text-gray-600 dark:text-gray-300">
				Nikmati layanan bordir, sablon, dan custom desain yang elegan, cepat, dan sesuai identitas
				brand Anda.
			</p>
		</div>

		<div class="grid grid-cols-1 gap-10 md:grid-cols-3">
			<!-- Card -->
			<div
				class="group rounded-3xl border border-transparent bg-white p-8 shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50 dark:hover:shadow-gray-900/80"
			>
				<div class="relative mb-6 overflow-hidden rounded-2xl">
					<img
						src="https://img.freepik.com/free-photo/portrait-businesswoman-owner_23-2148828328.jpg?w=740"
						alt="Jasa Bordir"
						class="h-60 w-full rounded-2xl object-cover transition duration-500 group-hover:scale-105"
					/>
					<div
						class="absolute top-4 left-4 rounded-full bg-blue-600 px-3 py-1 text-sm font-semibold text-white"
					>
						Bordir
					</div>
				</div>
				<h3 class="mb-2 text-2xl font-bold text-gray-800 dark:text-white">
					Jasa Bordir Profesional
				</h3>
				<p class="mb-4 text-gray-600 dark:text-gray-300">
					Tingkatkan identitas brand Anda dengan hasil bordir presisi, tahan lama, dan elegan untuk
					segala kebutuhan.
				</p>
				<a
					href="{base}/contact-us"
					class="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 transition hover:underline dark:text-blue-400"
				>
					Pelajari Lebih Lanjut
					<svg
						class="h-4 w-4"
						fill="none"
						stroke="currentColor"
						stroke-width="2"
						viewBox="0 0 24 24"
					>
						<path d="M17 8l4 4m0 0l-4 4m4-4H3" />
					</svg>
				</a>
			</div>

			<!-- Card -->
			<div
				class="group rounded-3xl border border-transparent bg-white p-8 shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50 dark:hover:shadow-gray-900/80"
			>
				<div class="relative mb-6 overflow-hidden rounded-2xl">
					<img
						src="https://img.freepik.com/premium-photo/close-up-person-working-textile-industry_1048944-24648174.jpg?w=740"
						alt="Jasa Sablon"
						class="h-60 w-full rounded-2xl object-cover transition duration-500 group-hover:scale-105"
					/>
					<div
						class="absolute top-4 left-4 rounded-full bg-blue-600 px-3 py-1 text-sm font-semibold text-white"
					>
						Sablon
					</div>
				</div>
				<h3 class="mb-2 text-2xl font-bold text-gray-800 dark:text-white">
					Sablon Kaos Berkualitas
				</h3>
				<p class="mb-4 text-gray-600 dark:text-gray-300">
					Layanan sablon dengan hasil warna tajam dan tahan lama, cocok untuk kaos komunitas,
					promosi, hingga distro.
				</p>
				<a
					href="{base}/contact-us"
					class="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 transition hover:underline dark:text-blue-400"
				>
					Pelajari Lebih Lanjut
					<svg
						class="h-4 w-4"
						fill="none"
						stroke="currentColor"
						stroke-width="2"
						viewBox="0 0 24 24"
					>
						<path d="M17 8l4 4m0 0l-4 4m4-4H3" />
					</svg>
				</a>
			</div>

			<!-- Card -->
			<div
				class="group rounded-3xl border border-transparent bg-white p-8 shadow-md transition duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50 dark:hover:shadow-gray-900/80"
			>
				<div class="relative mb-6 overflow-hidden rounded-2xl">
					<img
						src="https://img.freepik.com/free-photo/drawing-fabric_1098-18012.jpg?w=740"
						alt="Custom Desain Baju"
						class="h-60 w-full rounded-2xl object-cover transition duration-500 group-hover:scale-105"
					/>
					<div
						class="absolute top-4 left-4 rounded-full bg-blue-600 px-3 py-1 text-sm font-semibold text-white"
					>
						Custom
					</div>
				</div>
				<h3 class="mb-2 text-2xl font-bold text-gray-800 dark:text-white">Custom</h3>
				<p class="mb-4 text-gray-600 dark:text-gray-300">
					Bebaskan kreativitasmu! Bekerja sama dengan tim desain kami untuk menciptakan pakaian yang
					unik sesuai kebutuhan Anda.
				</p>
				<a
					href="{base}/contact-us"
					class="inline-flex items-center gap-2 text-sm font-semibold text-blue-600 transition hover:underline dark:text-blue-400"
				>
					Pelajari Lebih Lanjut
					<svg
						class="h-4 w-4"
						fill="none"
						stroke="currentColor"
						stroke-width="2"
						viewBox="0 0 24 24"
					>
						<path d="M17 8l4 4m0 0l-4 4m4-4H3" />
					</svg>
				</a>
			</div>
		</div>
	</div>
</section>

<!-- superiority Section -->
<section class="bg-white py-16 dark:bg-gray-900">
	<div class="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-32">
		<div class="mb-12 text-center">
			<h2 class="text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">Keunggulan Kami</h2>
			<p class="mx-auto mt-4 max-w-2xl text-lg text-gray-600 dark:text-gray-300">
				Konveksi Glam Stitch hadir dengan layanan terbaik mulai dari desain hingga produksi,
				memastikan setiap detail produk memenuhi ekspektasi Anda — cepat, berkualitas, dan
				fleksibel.
			</p>
		</div>
		<div class="grid grid-cols-1 gap-12 sm:grid-cols-2 lg:grid-cols-3">
			<!-- Asal Produksi -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Buatan Sendiri</dt>
				<dd class="mt-2 text-sm text-white">
					Semua produk dikerjakan langsung di workshop kami oleh tim ahli, bukan outsourcing.
				</dd>
			</div>

			<!-- Bahan -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Bahan Premium</dt>
				<dd class="mt-2 text-sm text-white">
					Kami gunakan kain terbaik seperti Cotton Combed, Polyester, dan Drill. Lembut dan awet.
				</dd>
			</div>

			<!-- Ukuran -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Ukuran Lengkap</dt>
				<dd class="mt-2 text-sm text-white">
					Tersedia semua ukuran dari XS sampai XXXXL. Bisa juga custom sesuai permintaan.
				</dd>
			</div>

			<!-- Finishing -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Finishing Rapi</dt>
				<dd class="mt-2 text-sm text-white">
					Setiap produk dijahit dengan rapi dan dicek ulang sebelum dikirim.
				</dd>
			</div>

			<!-- Paket -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Semua Sudah Termasuk</dt>
				<dd class="mt-2 text-sm text-white">
					Mulai dari desain, potong, jahit, sampai packing—semuanya sudah termasuk.
				</dd>
			</div>

			<!-- Catatan -->
			<div
				class="transform rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 p-8 shadow-lg transition-all duration-300 ease-in-out hover:scale-105"
			>
				<dt class="text-2xl font-semibold text-white">Catatan Penting</dt>
				<dd class="mt-2 text-sm text-white">
					Warna kain bisa sedikit beda karena layar & bahan. Tapi kami selalu jaga kualitas terbaik.
				</dd>
			</div>
		</div>
	</div>
</section>

<!-- Jam Kerja -->
<section class="bg-white py-12 sm:py-16 dark:bg-gray-900">
	<div class="mx-auto max-w-7xl px-6 lg:px-16">
		<div class="text-center">
			<h2 class="text-3xl font-bold text-gray-900 sm:text-4xl dark:text-white">Jam Kerja Kami</h2>
			<p class="mt-4 text-lg text-gray-600 dark:text-gray-300">
				Kami siap membantu Anda sesuai dengan jam kerja yang terjadwal di bawah ini.
			</p>
		</div>

		<div class="mt-10 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
			<!-- Senin - Sabtu -->
			<div
				class="flex items-center justify-center rounded-xl border border-transparent bg-white p-6 shadow-lg transition-transform duration-300 hover:scale-105 dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50"
			>
				<div class="text-center">
					<img
						src="https://media.istockphoto.com/id/1283403359/id/vektor/ikon-waktu-di-latar-belakang-transparan.jpg?s=612x612&w=0&k=20&c=foEv6n4528X0AkgcQZBX6TqWlZAOezpBEgoDRS-5LDo="
						alt="Senin-Sabtu"
						class="mx-auto mb-4 h-16 w-16 text-indigo-600"
					/>
					<h3 class="text-xl font-semibold text-gray-900 dark:text-white">Senin - Sabtu</h3>
					<p class="text-lg text-blue-500 dark:text-blue-400">09:00 - 15:00</p>
					<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
						Kami siap melayani Anda dari Senin hingga Sabtu, dari pukul 09:00 hingga 15:00. Anda
						bisa menghubungi kami untuk berbagai kebutuhan, mulai dari konsultasi hingga pemesanan
						produk.
					</p>
				</div>
			</div>

			<!-- Minggu -->
			<div
				class="flex items-center justify-center rounded-xl border border-transparent bg-white p-6 shadow-lg transition-transform duration-300 hover:scale-105 dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50"
			>
				<div class="text-center">
					<img
						src="https://img.freepik.com/premium-vector/closed-sign-icon-comic-style-accessibility-cartoon-vector-illustration-white-isolated-background-message-splash-effect-business-concept_157943-22063.jpg?ga=GA1.1.1868222757.1738365469&semt=ais_hybrid&w=740"
						alt="Minggu"
						class="mx-auto mb-4 h-16 w-16 text-indigo-600"
					/>
					<h3 class="text-xl font-semibold text-gray-900 dark:text-white">Minggu</h3>
					<p class="text-lg text-blue-500 dark:text-blue-400">Tutup</p>
					<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
						Minggu adalah hari libur bagi kami. Kami akan kembali melayani Anda pada hari Senin
						dengan semangat baru!
					</p>
				</div>
			</div>

			<!-- Layanan Online -->
			<div
				class="flex items-center justify-center rounded-xl border border-transparent bg-white p-6 shadow-lg transition-transform duration-300 hover:scale-105 dark:border-gray-700 dark:bg-gray-800 dark:shadow-gray-900/50"
			>
				<div class="text-center">
					<img
						src="https://media.istockphoto.com/id/1283403359/id/vektor/ikon-waktu-di-latar-belakang-transparan.jpg?s=612x612&w=0&k=20&c=foEv6n4528X0AkgcQZBX6TqWlZAOezpBEgoDRS-5LDo="
						alt="Layanan Online"
						class="mx-auto mb-4 h-16 w-16 text-indigo-600"
					/>
					<h3 class="text-xl font-semibold text-gray-900 dark:text-white">Layanan Online</h3>
					<p class="text-lg text-blue-500 dark:text-blue-400">Senin - Sabtu : 09:00 - 17:00</p>
					<p class="mt-2 text-sm text-gray-500 dark:text-gray-400">
						Tim customer service kami siap melayani Anda melalui chat online, WhatsApp, dan email
						dari Senin hingga Sabtu, pukul 09:00 - 17:00 WIB.
					</p>
				</div>
			</div>
		</div>
	</div>
</section>

<!-- Incentives -->
<section class="bg-white dark:bg-gray-900">
	<div class="mx-auto max-w-7xl px-4 py-24 sm:px-6 lg:px-16">
		<div
			class="rounded-2xl border bg-gray-50 px-6 py-16 sm:p-16 dark:border-gray-600 dark:bg-gray-900"
		>
			<div class="mx-auto max-w-xl lg:max-w-none">
				<div class="text-center">
					<h2 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">
						Konveksi Glam Stitch Mengutamakan Kepuasan Pelanggan
					</h2>
				</div>

				<div
					class="mx-auto mt-12 grid max-w-sm grid-cols-1 gap-x-8 gap-y-10 sm:max-w-none lg:grid-cols-3"
				>
					<!-- Produksi Cepat -->
					<div class="text-center sm:flex sm:text-left lg:block lg:text-center">
						<div class="sm:shrink-0">
							<div class="flow-root">
								<div
									class="mx-auto flex h-20 w-20 items-center justify-center text-6xl transition-transform duration-300 hover:scale-110 hover:animate-bounce"
								>
									🚛
								</div>
							</div>
						</div>
						<div class="mt-3 sm:mt-0 sm:ml-6 lg:mt-6 lg:ml-0">
							<h3 class="text-sm font-medium text-gray-900 dark:text-white">Produksi Cepat</h3>
							<p class="mt-2 text-sm text-gray-500 dark:text-gray-300">
								Konveksi Glam Stitch menawarkan produksi pakaian yang cepat dan tepat waktu untuk
								memenuhi tenggat waktu Anda tanpa mengurangi kualitas.
							</p>
						</div>
					</div>

					<!-- Jaminan Kualitas -->
					<div class="text-center sm:flex sm:text-left lg:block lg:text-center">
						<div class="sm:shrink-0">
							<div class="flow-root">
								<div
									class="mx-auto flex h-20 w-20 items-center justify-center text-6xl transition-transform duration-300 hover:scale-110 hover:animate-bounce"
								>
									✅
								</div>
							</div>
						</div>
						<div class="mt-3 sm:mt-0 sm:ml-6 lg:mt-6 lg:ml-0">
							<h3 class="text-sm font-medium text-gray-900 dark:text-white">Jaminan Kualitas</h3>
							<p class="mt-2 text-sm text-gray-500 dark:text-gray-300">
								Setiap produk dari Konveksi Glam Stitch melalui proses kontrol kualitas yang ketat
								untuk memastikan kepuasan pelanggan dan daya tahan produk.
							</p>
						</div>
					</div>

					<!-- Pesanan Kustom -->
					<div class="text-center sm:flex sm:text-left lg:block lg:text-center">
						<div class="sm:shrink-0">
							<div class="flow-root">
								<div
									class="mx-auto flex h-20 w-20 items-center justify-center text-6xl transition-transform duration-300 hover:scale-110 hover:animate-bounce"
								>
									🧵
								</div>
							</div>
						</div>
						<div class="mt-3 sm:mt-0 sm:ml-6 lg:mt-6 lg:ml-0">
							<h3 class="text-sm font-medium text-gray-900 dark:text-white">Pesanan Kustom</h3>
							<p class="mt-2 text-sm text-gray-500 dark:text-gray-300">
								Konveksi Glam Stitch menerima pesanan pakaian kustom dengan desain, bahan, dan
								spesifikasi ukuran Anda sendiri.
							</p>
						</div>
					</div>
				</div>
			</div>
		</div>
	</div>
</section>

<!-- Footer -->
<footer class="bg-gray-900" aria-labelledby="footer-heading">
	<h2 id="footer-heading" class="sr-only">Footer</h2>
	<div class="mx-auto max-w-7xl px-6 pt-16 pb-8 sm:pt-24 lg:px-8">
		<div class="animate__animated animate__fadeInUp xl:grid xl:grid-cols-3 xl:gap-8">
			<!-- Logo & Description -->
			<div class="space-y-6">
				<img
					class="animate__animated animate__zoomIn animate__delay-1s h-24 w-24"
					src={logoGlint}
					alt="Glam Stitch Logo"
				/>
				<div class="flex space-x-6">
					<!-- Facebook -->
					<a href="/" class="text-gray-500 transition duration-300 hover:text-gray-400">
						<span class="sr-only">Facebook</span>
						<svg
							class="animate__animated animate__fadeIn animate__delay-2s h-7 w-7"
							fill="currentColor"
							viewBox="0 0 24 24"
						>
							<path
								d="M22 12a10 10 0 10-11.6 9.87v-6.99H8.1v-2.88h2.3V9.41c0-2.28 1.35-3.55 3.42-3.55.99 0 2.03.18 2.03.18v2.24h-1.14c-1.12 0-1.47.7-1.47 1.42v1.71h2.5l-.4 2.88h-2.1v6.99A10 10 0 0022 12z"
							/>
						</svg>
					</a>

					<!-- Instagram -->
					<a href="/" class="text-gray-500 transition duration-300 hover:text-gray-400">
						<span class="sr-only">Instagram</span>
						<svg
							class="animate__animated animate__fadeIn animate__delay-2_5s h-7 w-7"
							fill="currentColor"
							viewBox="0 0 24 24"
						>
							<path
								d="M7 2C4.24 2 2 4.24 2 7v10c0 2.76 2.24 5 5 5h10c2.76 0 5-2.24 5-5V7c0-2.76-2.24-5-5-5H7zm10 2c1.66 0 3 1.34 3 3v10c0 1.66-1.34 3-3 3H7c-1.66 0-3-1.34-3-3V7c0-1.66 1.34-3 3-3h10zm-5 3a5 5 0 100 10 5 5 0 000-10zm0 2a3 3 0 110 6 3 3 0 010-6zm4.5-1a1 1 0 100 2 1 1 0 000-2z"
							/>
						</svg>
					</a>
				</div>
			</div>

			<!-- Navigation Links -->
			<div class="mt-16 grid grid-cols-2 gap-8 sm:grid-cols-3 xl:col-span-2 xl:mt-0">
				<!-- Navigasi -->
				<div>
					<h3 class="text-lg leading-6 font-semibold text-white">Navigasi</h3>
					<ul class="mt-6 space-y-4">
						<li>
							<a
								href="/"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Beranda</a
							>
						</li>
						<li>
							<a
								href="/product"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Produk</a
							>
						</li>
						<li>
							<a
								href="{base}/404.html"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Layanan</a
							>
						</li>
					</ul>
				</div>

				<!-- Products -->
				<div>
					<h3 class="text-lg leading-6 font-semibold text-white">Produk</h3>
					<ul class="mt-6 space-y-4">
						<li>
							<a
								href="/tshirt"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400">Kaos</a
							>
						</li>
						<li>
							<a
								href="/polo"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Kaos Polo</a
							>
						</li>
						<li>
							<a
								href="/jersey"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Jersey</a
							>
						</li>
						<li>
							<a
								href="/jacket"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Hoodie/Sweater</a
							>
						</li>
						<li>
							<a
								href="/shirts"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Kemeja</a
							>
						</li>
					</ul>
				</div>

				<!-- Discover Us -->
				<div>
					<h3 class="text-lg leading-6 font-semibold text-white">Jelajahi</h3>
					<ul class="mt-6 space-y-4">
						<li>
							<a
								href="/about-us"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Tentang Kami</a
							>
						</li>
						<li>
							<a
								href="/contact-us"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Hubungi Kami</a
							>
						</li>
						<li>
							<a
								href="/store-location"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Lokasi Toko</a
							>
						</li>
						<li>
							<a
								href="/gallery"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Tim Kami</a
							>
						</li>
						<li>
							<a
								href="{base}/404.html"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Pelanggan Kami</a
							>
						</li>
						<li>
							<a
								href="/supplier-kami"
								class="text-base text-gray-300 transition duration-200 hover:text-blue-400"
								>Supplier Kami</a
							>
						</li>
					</ul>
				</div>
			</div>
		</div>

		<!-- Footer Bottom -->
		<div
			class="animate__animated animate__fadeInUp animate__delay-3s mt-16 border-t border-white/10 pt-8 text-center"
		>
			<p class="text-sm leading-5 text-gray-400">&copy; 2025 Glam Stitch. All rights reserved.</p>
		</div>
	</div>
</footer>

<style type="text/tailwindcss"></style>
